...
#             'bathymetry_hypsometricmap',
    ...
    command='''
        ...
        (
            ( {{road}} -brightness-contrast 0%%x-12%% )
            ( {{label}} -channel A -morphology EdgeOut Disk:1  +channel -blur 2 ) -compose DstOut -composite
        ) -compose HardLight -composite

  # (
 #            {{label}} -geometry +2+2
 #            -channel A -virtual-pixel edge -blur 0x4
 #            -evaluate Multiply 0.3 +channel
 #            +level-colors black
